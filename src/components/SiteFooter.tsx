...
export default function SiteFooter({
  ...
  const linkCls =
    "text-[13px] font-semibold text-white/85 underline-offset-2 hover:text-white hover:underline";
  ...
  return (
    <footer className="no-print mt-6 bg-gov-footer text-white">
      <div className="mx-auto w-full max-w-5xl px-5 py-4">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-x-4 gap-y-1 border-t border-white/15 pt-3 text-[12px] text-white/70">
          <span>{t.site.copyright}</span>
          <span>{v.footOwner}</span>
        </div>
      </div>
    </footer>
  );
